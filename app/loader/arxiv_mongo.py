...
def stream_and_insert_data(
    collection, failures_collection, data_file_path: Path, batch_size: int
) -> int:
    """
    ...
    with open(data_file_path, "r", encoding="utf-8") as f:
                ...
                try:
                    ...
                    for e in bwe.details.get("writeErrors", []):
                        if failures_collection:
                            try:
                                failures_collection.insert_one(
                                    {"_id": e.get("op", {}).get("_id")}
                                )
                            except Exception:
                                pass
                    batch.clear()
        ...
        if batch:
            try:
                ...
                logger.warning(
                    f"[arxiv-job] BulkWriteError (마지막 배치): {bwe.details}"
                )
            except Exception as e:
                logger.error(
                    f"[arxiv-job] unexpected bulk_write error (마지막 배치): {e}"
                )
...
def seed_categories_from_mongo(collection) -> None:
    ...
    if unique_codes:
        logger.info(
            f"[arxiv-job] seeding PostgreSQL categories from {len(unique_codes)} codes"
        )
        try:
...
def ingest_arxiv_to_mongo() -> bool:
    ...
    collection = collection.with_options(write_concern=WriteConcern(w=1, j=False))
